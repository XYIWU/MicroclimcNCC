...
#' @return `snowtemp` if snow present, a data,frame of snow temperature (掳C), at each of the potential 8 snow layers (see details).
...
runmodelS <- function(climdata, vegp, soilp, nmrout, reqhgt,  lat, long, metopen = TRUE, windhgt = 2,
  ...
  if (reqhgt < hgt) {
    sel<-which(z>reqhgt)
    if (length(sel) > 1) {
      wgt1<-abs(z[sel[1]]-reqhgt)  # 冠层下制定位置之上的一个点距离要求大高度的距离
      wgt2<-abs(z[sel[1]-1]-reqhgt)  # 冠层下制定位置之下的一个点距离要求大高度的距离
      sel2<-c(sel[1]-1,sel)
      PAIu1<-vegp$PAI[sel,] ## 特定高度之上的总的PAI值
      if (length(sel) > 1) PAIu1<-apply(PAIu1,2,sum)
      PAIu2<-vegp$PAI[sel2,]  # 删选特定行 以及所有列
      if (length(sel2) > 1) PAIu2<-apply(PAIu2,2,sum)
      if (length(wgt2) > 1) {
        PAIu<-PAIu1+(wgt1/(wgt1+wgt2))*(PAIu2-PAIu1)
      } else PAIu<-PAIu1
...
runwithNMR <- function(climdata, prec, vegp, soilp, reqhgt, lat, long, altt = 0, slope = 0,
  ...
  LAI<-apply(LAI,2,mean)
  pLAI<-LAI/PAIt
  # LREFL: A single numeric value of average canopy reflectivity of shortwave radiation. 这里(1-pLAI)是什么意思？
  LREFL<-mean(pLAI*vegp$refls+(1-pLAI)*vegp$reflp)  # refls: reflectivity (shortwave radiation) of leaves;   reflp: Reflectivity of leaves to PAR
